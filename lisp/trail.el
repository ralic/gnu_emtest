;;;_ trail.el --- Trailed eval - Utility for WYSIWYG destructurer construction
...
(require 'emt-util) ;;For `emt/util/collect-in-tree'
(eval-and-compile
   (when (not (fboundp 'rtest:deftest))
...
(defun emt/trail:record-item (expression value)
   ...
   (let*
	 ...
	 (append emt/trail:value-list (list item)))
      ...
      (emt:testpoint tp:8efac1de-09d4-4865-bea2-d3df2b211ad3 ()
	 (let
	    ((buf
...
(defun emt/trail:build-code ()
   ...
   (let*
	 ...
	 (needed-items ()))
      ...
      ;;wasn't used)
      (emt:testpoint tp:9fd5ba68-6ce6-45ae-b79d-311eebbbfa56
	 (pending))
