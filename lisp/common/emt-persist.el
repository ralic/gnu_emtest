...
;;The only backend supported is persist.el.  
(require 'persist)
...
(deftype emt:db:record. () '(or emt:db:persist-archive. emt:db:group.))
;;;_  . The database list
(deftype emt:db:record-alist. () '(repeat (cons * emt:db:record.)))
...
(defalias 'emt:db:single:get-notes 'emt:db:single.-use-category)
...
;;;_  . Test supporter
;;Test supporters do not belong in t files.  They exist for callers'
;;testing benefit.
